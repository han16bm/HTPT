...
builder.Host.UseSerilog((ctx, cfg) =>
    cfg.ReadFrom.Configuration(ctx.Configuration)
       .WriteTo.Console()
       .WriteTo.File("logs/api-admin-.log", rollingInterval: RollingInterval.Day));
...
services.AddControllers(options =>
{
    ...
    options.Conventions.Add(new ApiPrefixRouteConvention("admin"));
...
{
    c.SwaggerDoc("v1", new() { Title = "API.Admin — Fish Shop", Version = "v1" });
});
...
app.UseSwaggerUI(c =>
{
    c.DocumentTitle = "API.Admin — Fish Shop";
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "API.Admin V1");
