...
export const productService = {
  search: (params: ProductSearchParams = {}) =>
    axiosClient.get<PagedResult<Product>>(API_ENDPOINTS.PRODUCT_SEARCH, {
      params,
    }),

  getById: (id: number | string) =>
    axiosClient.get<Product>(API_ENDPOINTS.PRODUCT_DETAIL, { params: { id } }),

  getBySlug: (slug: string) =>
    axiosClient.get<Product>(API_ENDPOINTS.PRODUCT_BY_SLUG, {
      params: { slug },
    }),

  upsert: (data: ProductUpsertRequest) =>
    axiosClient.post<Product>(API_ENDPOINTS.PRODUCT_UPSERT, buildFormData(data), {
      headers: { 'Content-Type': 'multipart/form-data' },
    }),

  delete: (id: number | string, reason?: string) =>
    axiosClient.post<void>(API_ENDPOINTS.PRODUCT_DELETE, { id, reason }),
};
