...
export const orderService = {
  ...
  getByCode: async (orderCode: string) => {
    const data = await axiosClient.get<OrderApiModel>(
      `${API_ENDPOINTS.ORDER_DETAIL}/${encodeURIComponent(orderCode)}`
    );
  ...
  updateStatus: (data: OrderUpdateStatusRequest) =>
    axiosClient.patch<Order>(
      `${API_ENDPOINTS.ORDER_UPDATE_STATUS}/${encodeURIComponent(data.orderCode)}/status`,
      data
    ),

  cancel: (data: { orderCode: string; reason?: string }) =>
    axiosClient.delete<Order>(`${API_ENDPOINTS.ORDER_CANCEL}/${encodeURIComponent(data.orderCode)}`, {
